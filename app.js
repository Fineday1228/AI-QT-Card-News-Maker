...
els.generateBtn?.addEventListener("click", async () => {
  try {
    ...
    const fullScript = inputValue(els.scriptInput).trim();
    ...
    const partnerName = extractPartnerName(fullScript);
    const eventTitle = inputValue(els.eventTitleInput).trim();
...
els.sampleBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (els.scriptInput) els.scriptInput.value = sampleScript;
    const images = makeSampleImages();
    const copies = buildCardCopy(sampleScript, DEFAULT_BRAND_NAME, extractPartnerName(sampleScript), "");
    const assignments = assignImages(images);
    generatedCards = [1, 2, 3, 4].map((page) => {
      const canvas = document.createElement("canvas");
      canvas.width = SIZE;
