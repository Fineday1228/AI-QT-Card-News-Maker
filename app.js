...
const SIZE = 1200;
const SAFE = 72;
const DEFAULT_BRAND_NAME = "AI Quantec";
...
const els = {
  ...
  cardTitleInputs: [1, 2, 3, 4].map((page) => document.querySelector(`#cardTitle${page}`)),
  cardBodyInputs: [1, 2, 3, 4].map((page) => document.querySelector(`#cardBody${page}`)),
  markupButtons: [...document.querySelectorAll(".markup-toolbar button")],
...
els.generateBtn?.addEventListener("click", async () => {
  try {
    ...
    await generateCardsFromCopies(collectCardCopies());
  } catch (error) {
    alert(error.message);
    setStatus("확인");
  } finally {
...
els.sampleBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    const copies = collectCardCopies();
    ...
    if (els.downloadCollageBtn) els.downloadCollageBtn.disabled = false;
    setStatus("완료");
  } finally {
...
els.cardBodyInputs.forEach((input) => {
  input?.addEventListener("focus", () => {
    activeMarkupInput = input;
  });
...
els.markupButtons.forEach((button) => {
  button.addEventListener("click", () => {
    const target = document.querySelector(`#${button.closest(".markup-toolbar")?.dataset.target}`);
    applyMarkupToSelection(target || activeMarkupInput, button.dataset.markup);
  });
});
...
function collectCardCopies() {
  const defaults = ["행사개요", "현장 스케치", "실전 투자 인사이트", DEFAULT_BRAND_NAME];
  return [0, 1, 2, 3].map((index) => ({
    title: inputValue(els.cardTitleInputs[index]).trim() || defaults[index],
    body: inputValue(els.cardBodyInputs[index]).trim(),
  }));
}

async function generateCardsFromCopies(copies) {
  generatedCards = [];
  const files = [...(els.imageInput?.files || [])];
  if (!files.length) throw new Error("세미나 사진을 1장 이상 선택하세요.");
  if (!copies.some((copy) => copy.body.trim())) throw new Error("카드에 들어갈 본문을 1개 이상 입력하세요.");

  const brandLogo = els.brandLogoInput?.files?.[0] ? await loadImageFile(els.brandLogoInput.files[0]) : null;
  const partnerLogo =
    els.partnerLogoToggle?.checked && els.partnerLogoInput?.files?.[0] ? await loadImageFile(els.partnerLogoInput.files[0]) : null;
...
function fillSampleCopies() {
  const sample = [
    {
      title: "AI 알고리즘 투자전략 세미나",
      body:
        "6월 12일 유진투자증권 본사 16층 HRD센터에서 AI Quantec이 주최한 투자전략 세미나가 개최되었습니다.\n" +
        "이번 세미나는 [bold green]데이터 기반 자산배분[/bold green] 전략과 AI 알고리즘 운용 구조를 공유한 시간이었습니다.",
    },
    {
      title: "현장 스케치",
      body:
    ...
    },
    {
      ...
      body:
        ...
        "[bold blue]기관 수준 리스크 관리[/bold blue]와 [bold green]데이터 기반 자산배분[/bold green] 전략의 가치가 다시 한번 조명되었습니다.",
    },
    {
      title: "AI Quantec",
      body:
        "세미나 말미에는 다양한 의견과 날카로운 질문이 이어졌습니다.\n" +
        "앞으로도 AI Quantec은 [bold green]기술과 데이터를 기반으로[/bold green] 여러분의 [bold blue]가장 강력한 투자 파트너[/bold blue]가 되겠습니다.",
    },
  ];
  sample.forEach((copy, index) => {
    if (els.cardTitleInputs[index]) els.cardTitleInputs[index].value = copy.title;
    if (els.cardBodyInputs[index]) els.cardBodyInputs[index].value = copy.body;
  });
}

function applyMarkupToSelection(input, type) {
  if (!input || !type) return;
  input.focus();
  const start = input.selectionStart ?? 0;
  const end = input.selectionEnd ?? start;
  const selected = input.value.slice(start, end) || "강조할 문구";
  const wrappers = {
    bold: ["**", "**"],
    green: ["[green]", "[/green]"],
    blue: ["[blue]", "[/blue]"],
    "bold-green": ["[bold green]", "[/bold green]"],
    "bold-blue": ["[bold blue]", "[/bold blue]"],
  };
...
function collectEventBrief() {
  return {
    title: inputValue(els.briefTitleInput).trim(),
    topic: inputValue(els.briefTopicInput).trim(),
    date: inputValue(els.briefDateInput).trim(),
    location: inputValue(els.briefLocationInput).trim(),
    host: inputValue(els.briefHostInput).trim() || DEFAULT_BRAND_NAME,
    partner: inputValue(els.briefPartnerInput).trim(),
    keyPoints: inputValue(els.briefKeyPointsInput).trim(),
    atmosphere: inputValue(els.briefAtmosphereInput).trim(),
    closing: inputValue(els.briefClosingInput).trim(),
  };
}
...
function normalizeEventInput(input) {
  ...
  if (!trimmed) return "";
  if (!looksLikeStructuredMemo(trimmed)) return trimmed;
  return buildScriptFromBrief(parseEventFields(trimmed));
}

function looksLikeStructuredMemo(text) {
  const fieldMatches = text.match(/(?:행사명|제목|주제|일시|날짜|장소|주최|브랜드|협력기관|협력사|파트너|핵심|내용|분위기|질의|마무리)\s*[:：]/g);
  return (fieldMatches || []).length >= 2;
}

function parseEventFields(text) {
  const aliases = {
    title: ["행사명", "제목", "타이틀"],
    topic: ["주제", "핵심 주제"],
    date: ["일시", "날짜", "일자"],
    location: ["장소", "위치"],
    host: ["주최", "브랜드", "주관"],
    partner: ["협력기관", "협력사", "파트너", "참여사"],
    keyPoints: ["핵심 내용", "내용", "주요 내용", "인사이트"],
    atmosphere: ["현장 분위기", "분위기", "참석자 반응"],
    closing: ["질의응답", "마무리", "토론"],
  };
  const fields = {};
  text.split(/\n+/).forEach((line) => {
    ...
    for (const [key, names] of Object.entries(aliases)) {
      if (names.some((name) => label.includes(name))) fields[key] = value;
    }
  });
  return { host: DEFAULT_BRAND_NAME, ...fields };
}

function buildScriptFromBrief(brief) {
  const title = brief.title || brief.topic || "AI Quantec 투자전략 세미나";
  const host = brief.host || DEFAULT_BRAND_NAME;
  const topic = brief.topic || "AI 기반 투자 전략과 데이터 기반 자산배분";
  const datePart = brief.date ? `${brief.date} ` : "";
  const locationPart = brief.location ? `${brief.location}에서 ` : "";
  const partnerPart = brief.partner ? `${brief.partner}과 함께 ` : "";
  const keyPoints = brief.keyPoints || "시장 변화에 대응하는 운용 전략과 실전 투자 인사이트";
  const atmosphere = brief.atmosphere || "참석자들은 높은 집중도로 발표를 경청하며 깊은 관심을 보였습니다";
  const closing = brief.closing || "세미나 말미에는 다양한 의견과 질문이 이어졌습니다";

  return [
    `${datePart}${locationPart}${host}이 주최한 ${title}가 개최되었습니다.`,
    `이번 세미나는 ${partnerPart}${topic}을 중심으로 진행되며 새로운 투자 방향성을 공유한 시간이었습니다.`,
    `현장에서는 ${atmosphere}.`,
    `${keyPoints}가 소개되며 참석자들의 관심은 더욱 높아졌습니다.`,
    `${closing}.`,
    `앞으로도 ${host}은 기술과 데이터를 기반으로 여러분의 가장 강력한 투자 파트너가 되겠습니다.`,
  ].join(" ");
}

function buildGeminiPrompt(brief) {
  return [
    "AI Quantec 공식 SNS 카드뉴스 제작을 위한 전체 행사 원고를 한국어로 작성하세요.",
    "입력 정보가 일부 부족해도 자연스럽고 신뢰감 있는 문장으로 보완하세요.",
    "과장된 홍보 문구는 피하고, 세미나 현장 후기 톤으로 작성하세요.",
    "날짜와 장소는 별도 배지가 아니라 문장 안에 자연스럽게 포함하세요.",
    "협력기관이 있으면 본문에서 1회만 자연스럽게 언급하세요.",
    "결과는 4장 카드뉴스 분해에 적합하도록 행사 개요, 현장 분위기, 핵심 인사이트, 질의응답/마무리 메시지를 모두 포함하세요.",
    "최종 출력은 제목 없이 전체 행사 원고 본문만 5~7문장으로 작성하세요.",
    "",
    `행사명/제목: ${brief.title || "미입력"}`,
    `주제: ${brief.topic || "미입력"}`,
    `일시: ${brief.date || "미입력"}`,
    `장소: ${brief.location || "미입력"}`,
    `주최/브랜드: ${brief.host || DEFAULT_BRAND_NAME}`,
    `협력기관: ${brief.partner || "없음"}`,
    `핵심 내용: ${brief.keyPoints || "미입력"}`,
    `현장 분위기: ${brief.atmosphere || "미입력"}`,
    `질의응답/마무리: ${brief.closing || "미입력"}`,
  ].join("\n");
}
...
async function generateScriptWithGemini(apiKey, prompt) {
  ...
  if (!text) throw new Error("Gemini가 원고를 반환하지 않았습니다.");
  return text.replace(/^```(?:text)?|```$/g, "").trim();
}

function updateImageFileSummary(files) {
  if (!els.imageFileSummary) return;
  if (!files.length) {
...
function extractPartnerName(script) {
  const normalized = normalizeSpace(script);
  const patterns = [
    /([A-Za-z][A-Za-z0-9-]{1,20})\s*(?:과|와)\s*함께/u,
    /(?:협력사|파트너|공동\s*주최|참여사)\s*[:：]?\s*([A-Za-z0-9가-힣][A-Za-z0-9가-힣&().·\-\s]{1,24})/u,
  ];
  for (const pattern of patterns) {
    const match = normalized.match(pattern);
    if (!match) continue;
    const candidate = match[1].trim().replace(/[,.，。]+$/g, "");
    if (candidate && candidate !== DEFAULT_BRAND_NAME) return candidate;
  }
  return "";
}
...
function tokenizeHighlights(text) {
  const parsedSegments = parseMarkupSegments(text);
  return parsedSegments.flatMap((segment) => {
    if (segment.color !== WHITE || segment.bold) {
      return splitKeepSpaces(segment.text, segment.color, segment.bold);
    }
    return tokenizeKeywordHighlights(segment.text);
  });
}

function parseMarkupSegments(text) {
  const segments = [];
  let index = 0;
  let buffer = "";
  let style = { color: WHITE, bold: false };
  ...
  while (index < text.length) {
    ...
    const openTag = rest.match(/^\[(green|blue|bold green|bold blue)\]/);
    const closeTag = rest.match(/^\[\/(green|blue|bold green|bold blue)\]/);
    if (openTag) {
      flush();
      style = styleForMarkup(openTag[1]);
      index += openTag[0].length;
      continue;
    }
    if (closeTag) {
      flush();
      style = { color: WHITE, bold: false };
...
function styleForMarkup(markup) {
  if (markup === "green") return { color: GREEN, bold: false };
  if (markup === "blue") return { color: BLUE, bold: false };
  if (markup === "bold green") return { color: GREEN, bold: true };
  if (markup === "bold blue") return { color: BLUE, bold: true };
  return { color: WHITE, bold: false };
}

function tokenizeKeywordHighlights(text) {
  const matches = [];
